...
static bool try_consume(const std::string& buf, size_t& pos, const std::string& expected) {
    if (pos + expected.size() > buf.size()) return false;
    if (buf.compare(pos, expected.size(), expected) != 0) return false;
    pos += expected.size();
    return true;
}
...
static bool try_parse_int(const std::string& buf, size_t& pos, int64_t& out) {
    size_t start = pos;
    if (pos >= buf.size() || !std::isdigit(buf[pos])) return false;
    out = 0;
...
static bool try_parse_bulk_str(const std::string& buf, size_t& pos, std::string& out) {
    size_t start = pos;
    ...
    int64_t len;
    if (!try_parse_int(buf, pos, len)) { pos = start; return false; }

    if (pos + len + 2 > buf.size()) { pos = start; return false; }
...
static bool try_parse_array(const std::string& buf, size_t& pos, std::vector<std::string>& out) {
    size_t start = pos;
    if (pos >= buf.size() || buf[pos] != '*') return false;
    pos++;

    int64_t count;
    if (!try_parse_int(buf, pos, count)) { pos = start; return false; }

    out.clear();
    for (int64_t i = 0; i < count; i++) {
        std::string elem;
        if (!try_parse_bulk_str(buf, pos, elem)) { pos = start; return false; }
...
std::optional<Command> RESPReader::try_parse() {
    ...
    if (!try_parse_array(buf_, pos, parts)) return std::nullopt;
